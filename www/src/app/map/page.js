...
export default function Threads ({ params, searchParams }) {
  ...
  const getMapConfig = useCallback(() => {
    const mapConfig = {
      true: {
        ...
        // borderRadius: isMobile ? '0px 0px' : '0px 0px 12px 12px'
        borderRadius: '0px'
      },
      false: {
        initialViewState: {
  ...
  return (<>
    <GlobalHeader />
    <main className="flex bg-[#F3F2EE] h-screen w-screen pt-16 md:p-4 md:pt-[82px]">
      <div className="flex flex-col bg-white border-[1.5px] w-full h-full md:rounded-xl shadow-xl">
        {/* <div className="bg-white md:rounded-t-xl border-[1.5px] w-full h-24 p-2">
          ...
          <Map
            ...
            initialViewState={getMapConfig().initialViewState}
            style={{ borderRadius: getMapConfig().borderRadius }}
