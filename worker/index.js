...
function main ({
  ...
  latestOutputPath = latestOutputPath || path.join(__dirname, '../api/', 'developments/snapshot.json')
  logsOutputPath = logsOutputPath || path.join(__dirname, '../api/', 'developments/logs.json')
  ...
  axios.get(siteUrl)
    .then(res => {
      ...
      fs.writeFileSync(latestOutputPath, JSON.stringify(data, null, 2))

      if (enableLogs && diff.length > 0) {
        fs.writeFileSync(logsOutputPath, JSON.stringify([...existingLogs, ...diff], null, 2))
      }
    })
    .catch(console.error)
}
